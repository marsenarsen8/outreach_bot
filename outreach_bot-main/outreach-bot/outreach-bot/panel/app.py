...
def get_broadcast_status():
    ...
    try:
        ...
        # Контакты для рассылки (NOT_SENT)
        cursor.execute('SELECT COUNT(*) FROM contacts WHERE status = "NOT_SENT"')
        not_sent_count = cursor.fetchone()[0]
        
        # Уже отправленные контакты
        cursor.execute('SELECT COUNT(*) FROM contacts WHERE status = "SENT"')
        sent_count = cursor.fetchone()[0]
        
        if total_contacts > 0:
            broadcast_progress["total"] = total_contacts
            broadcast_progress["sent"] = sent_count
            
            if broadcast_status == "running":
                broadcast_message = f"Рассылка выполняется. Отправлено: {sent_count}, Осталось: {not_sent_count}, Всего: {total_contacts}"
            elif broadcast_status == "stopped":
                if not_sent_count == 0:
                    broadcast_message = f"Рассылка завершена. Все контакты отправлены ({sent_count}/{total_contacts})"
                else:
                    broadcast_message = f"Рассылка остановлена. Отправлено: {sent_count}, Осталось: {not_sent_count}, Всего: {total_contacts}"
...
def start_broadcast():
    ...
    try:
        ...
        # Получаем подробную статистику по контактам
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        ...
        if not_sent_count == 0:
            status_info = f"Всего контактов: {total_contacts}, Отправлено: {sent_count}, Для рассылки: {not_sent_count}"
            return {"success": False, "message": f"Нет контактов для рассылки. {status_info}"}
        
        broadcast_status = "running"
        broadcast_progress["total"] = not_sent_count
        broadcast_progress["sent"] = 0
        broadcast_message = f"Рассылка запущена. Контактов для отправки: {not_sent_count} из {total_contacts} (уже отправлено: {sent_count})"
        ...
        return {"success": True, "message": broadcast_message}
    except Exception as e:
